  ...
  $.get(
    ...
    function (product) {
      ...
      function productInit(colorId) {
        ...
        let newPrice = chosenColor.price;
        let oldPrice = chosenColor.price
        let discount = chosenColor.discount
        let oldPriceElement = ''

        if (discount) {
          oldPriceElement = `<h3 class="old-price-details">$${oldPrice.toFixed(2)}</h3>`
        ...
        product.colors.forEach((color, index) => {
          ...
          colorOption.onmouseover = function () {
            $(".color")
              .eq(index)
              .css("outline", " 3.5px solid " + color.colorCode);
        ...
        $(".chosen").css(
          "box-shadow",
          " 0 0 0 3px white, " + "0 0 0 8px" + chosenColor.colorCode
        );
