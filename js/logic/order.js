...
function saveDetails() {
  ...
  if (Object.values(address).includes("")) {
    ...
    document
      .querySelectorAll("#address-form input")
...
// www.gympowers.link;
  $.get(
    "http://localhost:8080/order/current/products?" +
      $.param({ email: email }),
    function (responseJson) {
      ...
      addressContainer.innerHTML =
        ...
        ", " +
        responseJson.address.postCode +
        "</div>" +
        "       <div>" +
        responseJson.address.country +
        "</div>";
      ...
      $.each(responseJson.orderItems, function (index, orderItem) {
        ...
        $(".color-visual")
          .eq(index)
          .css("background-color", orderItem.colorCode);
      });
