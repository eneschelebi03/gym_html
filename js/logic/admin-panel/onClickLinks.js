...
function loadCustomers() {
  ...
  // www.gympowers.link;
  $.get(
    "http://localhost:8080/admin-panel/all/customers",
    function (customers) {
  ...
  $.get("http://localhost:8080/admin-panel/all/products", function (products) {
    ...
    products.forEach(function (product) {
      ...
      categoriesCell.appendChild(
        document.createTextNode(product.categories.join(", "))
      );

      const measurementsCell = newRow.insertCell(3);
      measurementsCell.appendChild(
        document.createTextNode(product.options.join(", "))
      );
